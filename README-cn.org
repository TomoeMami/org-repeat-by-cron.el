...
- =REPEAT_DEADLINE= : *（可选）* 设置为 =t= 时，更新 =DEADLINE= 而非 =SCHEDULED= 。 *注意：* 为了防止冲突，启用此项后会自动清除任务原有的 =SCHEDULED= 时间戳。
- =REPEAT_ANCHOR= : 由插件自动创建并维护，存储上次触发的时间基准。用户通常不需要手动干预。若现有的该项被删除，则在下次重复时会采用当前时间为基准，并重新计算设置该值。
*** 工作流程
...
- *自动发生* :
  ...
  - =org-repeat-by-cron= 会捕获到状态变更，并暂存 =repeater= （如果有的话）。
    
  - 它会根据 =REPEAT_CRON= 规则计算出下一个有效的时间点，忽略掉 =repeater= 的设置（如果有的话）。
    
  - 更新任务的 =SCHEDULED= 或 =DEADLINE= 时间戳，并恢复 =repeater= （如果有的话）。
    
  - 将任务的 =TODO= 状态从 =DONE= 自动重置为 =TODO= （或您的默认初始状态）。
    
提示：如果将 =org-repeat-by-cron= 与内置的 =org-repeater= 标记同时使用，则会暂存并忽略 =repeater= ，由 =REPEAT_CRON= 控制下次重复的时间点，恢复旧有的 =repeater= 。这意味着可以与 =org-habit= 同时启用，在保留 =repeater-cookie= 不变的同时，由 =org-repeat-by-cron= 接管日期计算， =repeater-cookie= 负责触发 Habit 属性（如进度条）。
