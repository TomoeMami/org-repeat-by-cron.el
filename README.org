...
Improved logic for determining when to include time in the timestamp. It will use =%Y-%m-%d %a %H:%M= if:
1. The cron rule has 5 fields.
2. The =anchor-str= contains a time (=HH:MM=).
3. The existing =SCHEDULED= or =DEADLINE= contains a time.
